...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = sites.join(ranks).flatMapToPair(t -> {
            int websiteId = t._1();
            Tuple2<Website, Double> value = t._2();
            ...
            while (edgeIter.hasNext()) {
                int dest = edgeIter.next();
                contribs.add(new Tuple2<>(dest, currentRank / edges.getNEdges()));
            }
            return contribs;
        });
        return newRanks.reduceByKey((r1, r2) -> r1 + r2).mapValues(r -> 0.15 + 0.85*(r));
    }
}
